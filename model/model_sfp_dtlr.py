...
class RECLIPPP(_BaseRECLIPPP):
    ...
    def sfp_logit_purify(self, output, sfp_score):
        ...
        with torch.no_grad():
            ...
            if getattr(self, "sfp_debug_export", False):
                ...
                # attention_discrepancy == sfp_score; combined URD == rank_score
                # (equals sfp_score when margin disabled, the shipped operating point).
                self.sfp_debug_maps["normalized_entropy"] = h_norm.detach().float().cpu()
                self.sfp_debug_maps["combined_urd_score"] = rank_score.reshape(B, H, W).detach().float().cpu()
                if getattr(self, "sfp_entropy_gate", False):
                    m_u = (flat_h_norm > float(self.sfp_entropy_tau_unrel))
                else:
                    m_u = (flat_conf < float(self.sfp_conf_thd))
                self.sfp_debug_maps["unreliable_mask_Mu"] = m_u.reshape(B, H, W).detach().float().cpu()
        ...
        if getattr(self, "sfp_proxy_enable", False):
            ...
            if getattr(self, "sfp_debug_export", False):
                self.sfp_debug_maps["high_conf_proxy_mean"] = proxy_mean.detach().float().cpu()
                self.sfp_debug_maps["high_conf_mask"] = high_conf.squeeze(1).detach().float().cpu()
                self.sfp_debug_maps["proxy_available"] = proxy_available.squeeze(1).detach().float().cpu()
        ...
        if getattr(self, "sfp_debug_export", False):
            ...
            self.sfp_debug_maps["local_neighbor_proxy"] = neigh_mean.detach().float().cpu()
            self.sfp_debug_maps["clean_target"] = output_clean.detach().float().cpu()
            self.sfp_debug_maps["purified_logits"] = output_new.detach().float().cpu()
            self.sfp_debug_maps["purification_update"] = (output_new - output).detach().float().cpu()
    ...
    def sfp_domain_transform_logit_refine(self, output, image):
        ...
        if getattr(self, "sfp_dtlr_structure_protect_enable", False):
            ...
            if getattr(self, "sfp_debug_export", False):
                self.sfp_debug_maps["structure_mask"] = structure_mask.squeeze(1).detach().float().cpu()
        ...
        if getattr(self, "sfp_debug_export", False):
            ...
            # dtlr_input == logits entering DTLR (== purified logits); rgb_guide is
            # the model-input image (dataset-normalized); the filter internally
            # min-max normalizes it to [0,1] at logit resolution (different semantics).
            self.sfp_debug_maps["dtlr_input"] = output.detach().float().cpu()
            self.sfp_debug_maps["rgb_guide"] = image.detach().float().cpu()
            self.sfp_debug_maps["dtlr_residual"] = delta.detach().float().cpu()
            self.sfp_debug_maps["dtlr_residual_magnitude"] = delta.abs().mean(dim=1).detach().float().cpu()
            self.sfp_debug_maps["mask_gated_update"] = (output_new - output).detach().float().cpu()
            self.sfp_debug_maps["structure_aware_logits"] = output_new.detach().float().cpu()
    ...
    def forward(self, image, gt_cls, zeroshot_weights, cls_name_token, training=False, img_metas=None,
        ...
        # --- J-VISUAL-EVIDENCE-2A debug export (default OFF; read-only capture) ---
        # Guarded by sfp_debug_export so OFF forward is byte-identical: only
        # DETACHED cpu copies of already-computed tensors are stored, no compute.
        if getattr(self, "sfp_debug_export", False):
            self.sfp_debug_maps["input_rgb"] = image.detach().float().cpu()
            self.sfp_debug_maps["initial_logits"] = output.detach().float().cpu()
            self.sfp_debug_maps["initial_pred"] = output.argmax(dim=1).detach().cpu()
            if self._sfp_attn_cache is not None:
                self.sfp_debug_maps["raw_attention"] = self._sfp_attn_cache.detach().float().cpu()
        ...
        if getattr(self, "sfp_debug_export", False):
            self.sfp_debug_maps["final_logits"] = output.detach().float().cpu()
            self.sfp_debug_maps["final_pred"] = output.argmax(dim=1).detach().cpu()
