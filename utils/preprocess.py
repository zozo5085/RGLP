...
def _open_pseudo_label(path):
    """Open a pseudo-label list file if it exists.

    The pseudo-label lists are a training-time artifact. The evaluation entry
    points (tools/test.py, tools/test_tta.py) discard the returned value, so a
    missing file must not stop inference. Present -> identical behavior to
    open(path, 'r'); absent -> iterate over nothing.
    """
...
def read_file_list(cfg):
    ...
    if dataset == 'context':
        ...
        with _open_pseudo_label(json_file_path) as file:
            for line in file:
                line = line[1:-2]
                if ',' in line:
